...
class DijsktraNode:
    ...
    def __lt__(self, other: DijsktraNode) -> bool:
        return self.distance < other.distance

    def __eq__(self, other: DijsktraNode) -> bool:
        return self.distance == other.distance
...
def dijkstra(wg: WeightedGraph['V'], root: V) -> Tuple[List[Optional[float]], Dict[int, WeightedEdge]]:
    ...
    pq: PriorityQueue[DijsktraNode] = PriorityQueue()
    pq.push(DijsktraNode(first, 0))

    while not pq.empty:
        u: DijsktraNode = pq.pop().vertex
        ...
        for we in wg.edges_for_index(u):
            ...
            if dist_v is None or dist_v > dist_u + we.weight:
                ...
                path_dict[we.v] = we
                pq.push(DijsktraNode(we.v, distances[we.v]))
